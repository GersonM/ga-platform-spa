...
const EntityFieldSelector = ({placeholder, mode, style, ...props}: EntityFieldSelectorProps) => {
  ...
  return (
    <Select
      {...props}
      allowClear
      placeholder={placeholder || 'Selecciona una tipo'}
      showSearch={true}
      ...
      optionRender={option => {
        return <div>
          ...
          <br/>
          {/* @ts-ignore */}
          <small>{option?.data.entity?.description || option?.data.entity?.code}</small>
        </div>;
      }
      }
    />
